...
export class SubscriberService {
  ...
  async getSubscribers(options = {}) {
    try {
      ...
      // For debugging: Just get ALL documents with no filters/ordering/limits
      console.log("📡 Using simple query with no filters...");
      ...
      const result = querySnapshot.docs.map((doc) => {
        ...
        return {
          ...
          subscribedAt: data.subscribedAt || data.subscribed_at,
          status: data.status || SUBSCRIBER_STATUS.ACTIVE, // Default to active if no status
        };
      });
