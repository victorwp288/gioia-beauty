...
export const warmAppointmentCache = async () => {
  try {
    // Import dataManager instead of using passed appointmentService
    ...
    const todayAppointments = await dataManager.getAppointmentsByDate(today);
    cacheAppointmentsByDate(today.toISOString(), todayAppointments);

    // Warm tomorrow's appointments
    const tomorrowAppointments = await dataManager.getAppointmentsByDate(
      tomorrow
    );
    cacheAppointmentsByDate(tomorrow.toISOString(), tomorrowAppointments);

    // Warm recent appointments list
    const recentAppointments = await dataManager.getAppointments({
      dateRange: {
        start: today,
        end: nextWeek,
      },
