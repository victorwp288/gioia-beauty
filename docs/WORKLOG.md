...
**Verified/reconciled:** the immediate pooler probe failed during propagation as documented; the single fresh post-window probe authenticated and returned exactly one `open` maintenance row with a positive version. The Vercel UI confirmed the updated variable remains Sensitive, Preview-only, and scoped to `refactor`. Exact-head Preview `dpl_DceLfm93u77jmnco4ywVvf25hduz` at `e9089a9` reached READY; its immutable protected URL returned health 200 `status=ok` and maintenance 200 with both mutation gates open, and PII-free runtime logs recorded successful `refactor` route completions.
...
**Next:** Keep Firebase/Production untouched. Continue the open Phase 3 privacy/retention/DPA package and Preview observability validation; any Production configuration, real-source read, or cutover remains separately approved.
...
**Data impact:** zero database/Auth/Firebase/customer rows; three sensitive Preview variables; six PostHog route events and three Sentry surrogates from bounded synthetic Preview GETs
...
**Expected reads/writes/rows:** provider/account metadata reads; two fresh project/config writes; three Preview/refactor variable writes; three bounded maintenance reads returned 503; 9 synthetic provider events; 0 business rows
**Done:** Commit `d7e12c6` adds strict server-only adapters, fixed surrogate errors, personless route metrics, EU/exact-bundle startup gates, 750 ms fail-open deadline, real SDK-envelope privacy proof, client-boundary guards, runbooks, and a Production-ready variable contract. Deferred project-wide Vercel bot controls because they would also change live Production; retained existing Turnstile/rate limits and automatic DDoS protection.
**Verified/reconciled:** 189 test files/2,249 tests; focused 75 tests; typecheck, lint, format, build, DB static, 56 timezone tests in both zones, real Sentry/PostHog envelope tests, built-client provider scan, and independent review pass. Deployment `dpl_6S6JYr9QiCF7DmefND8kfc9tGY5D` is READY at exact SHA `d7e12c6`; Vercel logs and provider UIs prove six fixed PostHog events and three fixed Sentry events with no stack/request/customer data. Dependency audit confirmed only pre-existing Resend/brace-expansion highs plus Firebase-admin/uuid moderates; local gitleaks 8.30.1 is unavailable.
...
**Target:** Vercel `gioia-beauty` Preview deployments `dpl_bkH62gyhYYhdjvNSzHTcZ7bNr7eR` for hosted Auth and `dpl_6Yc63yYk9S9uzyjQ5RReq3T6UMpg` for exact commit `428eb14`, TEST Supabase `hzibzwhrwmljgjjdzspi`; Production/Firebase/Resend untouched
...
**Done:** Commit `428eb14` replaced only `refactor` Preview bindings, corrected newline-sensitive CA/DSN bytes, proved server-only `app_runtime`, owner origin/CSRF/cookie/logout/replay behavior, and controlled public `503`; cleanup permits only one password AMR claim tied by session FK to the exact synthetic owner. A registry-drift follow-up updates Next 15.5.21, Sharp 0.35.3, and fast-xml-parser 5.10.1 without `--force`.
**Verified/reconciled:** Both Vercel builds READY; exact-head `/api/health` 200 and `/api/maintenance` 200; hosted owner flow passed, public availability 503 fail-closed, final TEST operational/Auth/Storage rows zero; local 170 files/2,103 tests, TS7/TS6, lint, format, DB static, production build, zero high dependency advisories, and diff checks pass
...
**Next:** Implement the remote-safe Preview browser harness and distributed challenge adapter. Keep source-snapshot `[PROD-READ]`, Sentry/email provider activation, privacy/legal decisions, and production source freeze separately approved.
