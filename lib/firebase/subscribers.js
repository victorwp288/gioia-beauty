...
export class SubscriberService {
  ...
  async getSubscribers(options = {}) {
    try {
      console.log("📡 Fetching subscribers from Firebase with safe limits...");

      // Start with simple query to avoid index issues
      let q = query(this.collectionRef);

      // Apply status filter if provided (this works without additional indexes)
      if (options.status) {
        q = query(q, where("status", "==", options.status));
      }

      // Skip date filtering for now to avoid index issues
      // if (options.startDate) {
      //   q = query(q, where("subscribedAt", ">=", options.startDate));
      // }
      // if (options.endDate) {
      //   q = query(q, where("subscribedAt", "<=", options.endDate));
      // }

      // Apply reasonable limit to prevent massive reads
      const limit_value = options.limit || 1000;
      const safeLimit = Math.min(limit_value, 1000);
      q = query(q, limit(safeLimit));

      console.log(
        "📡 Executing simple subscriber query with limit:",
        safeLimit
      );
      ...
      const result = querySnapshot.docs.map((doc) => {
        ...
        return {
          id: doc.id,
          ...data,
          // Normalize field names - prioritize subscribed_at from database
          subscribedAt:
            data.subscribed_at ||
            data.subscribedAt ||
            data.createdAt ||
            new Date().toISOString(),
          status: data.status || SUBSCRIBER_STATUS.ACTIVE,
        };
      });
      ...
      try {
        ...
        const simpleQuery = query(this.collectionRef, limit(1000));
        const querySnapshot = await getDocs(simpleQuery);

        const result = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
          status: doc.data().status || SUBSCRIBER_STATUS.ACTIVE,
          subscribedAt:
            doc.data().subscribed_at ||
            doc.data().subscribedAt ||
            doc.data().createdAt ||
            new Date().toISOString(),
        }));
  ...
  async batchDeleteSubscribers(ids) {
    try {
      const chunkSize = 450; // Keep below Firestore's 500 ops batch limit
      const idChunks = [];
      ...
      // Invalidate cache after deleting subscribers
      invalidateTags("subscribers");
