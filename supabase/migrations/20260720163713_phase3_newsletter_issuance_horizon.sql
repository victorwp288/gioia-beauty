...
create or replace function gioia_private.subscribe_public_newsletter(
  p_principal_scope_hash bytea, p_idempotency_key text,
  p_request_fingerprint bytea, p_email text
)
returns table (http_status smallint, result jsonb, replayed boolean)
language plpgsql volatile security definer set search_path = '' as $$
declare
  v_command_id uuid; v_replayed boolean; v_status smallint; v_stored jsonb;
  v_email text; v_subscriber gioia_private.newsletter_subscribers%rowtype;
...
begin
  ...
  where signing_key.issue_enabled
    and signing_key.verify_until >= v_now + interval '31 days';
  ...
  ) on conflict (email) do nothing returning * into v_subscriber;
  if found then
    v_changed := true;
  else
    select subscriber.* into strict v_subscriber
    from gioia_private.newsletter_subscribers as subscriber
    where subscriber.email = v_email::extensions.citext for update;
    if v_subscriber.status in ('legacy_unverified', 'unsubscribed', 'bounced') then
      ...
      join gioia_private.newsletter_action_tokens as token
        on token.consent_cycle_id = cycle.id
      ...
      order by token.issue_sequence desc limit 1;
  ...
  if v_changed then
    ...
    insert into gioia_private.newsletter_consent_cycles (
      ...
      v_artifact.policy_version, v_artifact.artifact_version,
    ...
    insert into gioia_private.newsletter_action_tokens (
      ...
      extensions.gen_random_uuid(), v_cycle.id, v_subscriber.id,
      v_subscriber.version, 'newsletter_confirm', v_key.key_id,
    ...
    insert into gioia_private.newsletter_consent_events (
      ...
      v_cycle.id, v_subscriber.id, v_subscriber.version,
      'requested', v_command_id
    );
    ...
    insert into gioia_private.email_outbox (
      ...
      pg_catalog.jsonb_build_object(
        ...
        'consentArtifactSha256',
          pg_catalog.encode(v_artifact.content_sha256, 'hex'),
        'action', pg_catalog.jsonb_build_object(
          ...
          'issuedAt', pg_catalog.to_char(
            v_token.issued_at at time zone 'UTC',
            'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'
          ),
          'expiresAt', pg_catalog.to_char(
            v_token.expires_at at time zone 'UTC',
            'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'
          ),
