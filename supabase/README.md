...
- Immediately after the one successful credential probe, the operator waits a separate unconditional 125 seconds for Supavisor's observed 120-second idle backend timeout, then requires zero application sessions before rebuild. No backend termination SQL exists because Supavisor rewrites the probe's `PGAPPNAME` to `application_name=Supavisor`; authentication is still never retried.
