...
- The reviewed hosted pgTAP transport keeps each file in one rollback-only transaction but sends `080_subscriber_commands.test.sql` in two explicit query messages at its committed marker, with a 5 ms pause beyond the domain's canonical millisecond precision. This gives its compressed re-subscription scenario a fresh PostgreSQL `statement_timestamp()` like separate user requests and the local CLI runner; the split is filename-pinned, digest-pinned, and rejected if missing, duplicated, or added to another file.
