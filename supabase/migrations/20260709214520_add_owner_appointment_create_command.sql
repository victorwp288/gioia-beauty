...
create function gioia_private.owner_create_appointment(
  ...
  begin
    if p_client_name is null
      ...
      or (p_client_email is not null and (
        pg_catalog.length(pg_catalog.btrim(p_client_email)) not between 3 and 320
        or pg_catalog.strpos(p_client_email, '@') < 2
      ))
      or (p_client_phone is not null
        and pg_catalog.length(pg_catalog.btrim(p_client_phone)) not between 1 and 40)
      or (p_client_note is not null and pg_catalog.length(p_client_note) > 2000) then
      raise sqlstate 'PT400' using message = 'APPOINTMENT_CONTACT_INVALID';
    end if;
    ...
    insert into gioia_private.schedule_entries (
      ...
      client_note, created_by
    ) values (
      'appointment', 'confirmed', 'admin', p_local_date, p_start_minutes,
      v_variant.duration_minutes, v_variant.buffer_minutes,
      p_service_id, p_variant_id, v_variant.service_name,
      v_variant.variant_name, v_variant.price_cents, v_variant.currency,
      pg_catalog.btrim(p_client_name),
      case when p_client_email is null then null else
    ...
    perform gioia_private.record_domain_change(
      'schedule_entry', v_entry.id, v_entry.version, 'create', 'admin',
...
revoke all on function gioia_private.owner_create_appointment(
  uuid, text, bytea, date, smallint, text, text, text, text, text, text
) from public, anon, authenticated, service_role, gioia_migrator;
grant execute on function gioia_private.owner_create_appointment(
  uuid, text, bytea, date, smallint, text, text, text, text, text, text
) to app_runtime;
