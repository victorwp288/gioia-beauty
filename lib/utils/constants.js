...
import {
  BUSINESS_INFO as VERIFIED_BUSINESS_INFO,
  SITE_URL,
  formattedAddress,
...
export const BUSINESS_INFO = {
  NAME: VERIFIED_BUSINESS_INFO.name,
  ADDRESS: formattedAddress(),
  PHONE: VERIFIED_BUSINESS_INFO.phoneDisplay,
  EMAIL: VERIFIED_BUSINESS_INFO.email,
  WEBSITE: SITE_URL,
...
import lpgData from "@/data/lpgData.js";
...
const alignArrayLength = (arr, targetLength, fallbackValue) => {
  if (targetLength <= 0) return [];
  const source = Array.isArray(arr)
    ? arr.filter((n) => typeof n === "number" && !Number.isNaN(n))
    : [];
  if (source.length === targetLength) return source;
  if (source.length === 0) return Array(targetLength).fill(fallbackValue);
  if (source.length === 1) return Array(targetLength).fill(source[0]);
  const padded = [...source];
  while (padded.length < targetLength) {
    padded.push(source[source.length - 1]);
  }
  return padded.slice(0, targetLength);
};

const buildVariantLabels = (option, service, durations, typeName) => {
  const raw = (() => {
    if (Array.isArray(option?.variants) && option.variants.length > 0)
      return option.variants;
    if (typeof option?.variants === "string") return [option.variants];
    if (Array.isArray(service?.variants) && service.variants.length > 0)
      return service.variants;
    if (typeof service?.variants === "string") return [service.variants];
    return [];
  })();

  if (raw.length === 0) {
    return durations.map((duration) =>
      durations.length > 1 ? `${typeName} ${duration} minuti` : typeName
    );
  }

  if (raw.length === durations.length) return raw;
  if (raw.length === 1) return Array(durations.length).fill(raw[0]);

  const padded = [...raw];
  while (padded.length < durations.length) {
    padded.push(raw[raw.length - 1]);
  }
  return padded.slice(0, durations.length);
};

const DEFAULT_EXTRA_TIME = 5;
...
const convertDataToAppointmentTypes = () => {
  ...
  try {
    categories = [
      ...
      { data: laserData || [], category: "laser", categoryName: "Laser" },
      {
        data: lpgData || [],
        category: "lpg",
        categoryName: "Lpg Endermologie",
      },
    ];
  } catch (error) {
  ...
  categories.forEach(({ data, category, categoryName }) => {
    ...
    sortedData.forEach((service) => {
      if (!service || service.includeInBooking === false) return;

      const rawOptions = Array.isArray(service.bookingOptions)
      ...
      const optionsToProcess =
        rawOptions.length > 0
          ...
          : [
              {
                type: service.title,
                durations: service.durations ?? service.duration,
      ...
      optionsToProcess.forEach((option, optionIndex) => {
        if (!option || option.includeInBooking === false) return;

        const typeName =
          option.type ||
          (service.title
            ? `${service.title}${
                optionsToProcess.length > 1 ? ` (${optionIndex + 1})` : ""
              }`
            : null);

        if (!typeName) return;

        const durationsParsed = extractDurations(
          option.durations ??
            option.duration ??
            service.durations ??
            service.duration
        );
        if (durationsParsed.length === 0) return;

        const extraTimeParsed = alignArrayLength(
          extractDurations(option.extraTime ?? service.extraTime),
          durationsParsed.length,
          DEFAULT_EXTRA_TIME
        );

        const variantLabels = buildVariantLabels(
          option,
          service,
          durationsParsed,
          typeName
        ...
        const description =
          option.description ||
          service.description ||
          `${typeName} - ${durationsParsed[0]} minuti`;
        ...
        groupedServices[typeName] = {
          ...
          basePrice:
            option.basePrice ?? service.basePrice ?? DEFAULT_BASE_PRICE,
          description,
          category,
          categoryName,
          active: option.active ?? service.active ?? true,
        };
      });
    });
  });
...
export const NOTIFICATION_CONFIG = {
  ...
  REPLY_TO: VERIFIED_BUSINESS_INFO.email,
};
