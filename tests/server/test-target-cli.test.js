...
describe("greenfield TEST Supabase CLI", () => {
  ...
  it("passes all 25 reviewed remote pgTAP files explicitly", async () => {
    const { run } = fakeRunner();
    const database = fakePgTapDatabase();
    const result = await client(run, database.client).runRemotePgTap();
    const suite = remotePgTapFiles();

    expect(result).toMatchObject({ assertions: 431, files: suite.files });
    ...
    expect(testCalls).toHaveLength(25);
    expect(
      testCalls.every(
