...
- Immediately after the one successful credential probe, operator cleanup may reap at most one lingering client backend matching the current database, exact `app_runtime` user, and exact `gioia_greenfield_credential_probe` application name. Multiple matches, termination failure, `gioia_public_api`, or any unknown application session fail closed before rebuild; no authentication retry is allowed.
