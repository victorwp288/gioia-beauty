...
const convertDataToAppointmentTypes = () => {
  ...
  categories.forEach(({ data, category, categoryName }) => {
    ...
    sortedData.forEach((service, index) => {
      ...
      const mainDuration = parseInt(service.duration) || 60;
      ...
      if (service.subcategories && service.subcategories.length > 0) {
        service.subcategories.forEach((sub) => {
          if (sub.duration && sub.duration !== "+10") {
  ...
  try {
    if (Array.isArray(appointmentTypesJSON)) {
      appointmentTypesJSON.forEach((appointment, index) => {
        if (
          appointment &&
          appointment.type &&
          !seenServiceNames.has(appointment.type)
        ) {
          seenServiceNames.add(appointment.type);
          groupedServices[appointment.type] = {
            type: appointment.type,
            durations: Array.isArray(appointment.durations)
              ? appointment.durations
              : [60],
            variants: [appointment.type],
            extraTime: Array.isArray(appointment.extraTime)
              ? appointment.extraTime
              : [5],
            basePrice: 50,
            description: `${appointment.type} - ${
              appointment.durations?.[0] || 60
            } minuti`,
            category: "general",
    ...
    console.warn("⚠️ Error loading appointmentTypes.json:", error);
  }
...
export const getAppointmentType = (typeName) => {
  return Object.values(APPOINTMENT_TYPES).find(
