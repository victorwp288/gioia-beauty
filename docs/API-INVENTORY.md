...
<!-- api-route {"method":"POST","path":"/api/admin/subscribers/unsubscribe"} -->
...
<!-- api-route {"method":"POST","path":"/api/admin/vacations/update"} -->
...
<!-- api-route {"method":"GET","path":"/api/maintenance"} -->
...
| `GET /api/maintenance`             | Public; request body/query are ignored                                                                                                                                                      | Current `open`/`frozen`/`owner_reconcile` mode, version, and fixed Italian customer/owner message; no PII | No app limiter; private no-store and noindex                            | 1 database transaction/function returning exactly 1 control row; 0 writes/Auth/provider calls                                                                                                                                                                                                        |
| `GET /api/availability`            | Public; query ≤1 KiB with exactly one `date`, `serviceId`, and `variantId`; duplicates and unknowns rejected before the abuse guard                                                         | Date/catalog IDs plus ≤96 ordered slot integers or fixed guard error; no PII                              | Durable network bucket; Preview requires the Turnstile bundle; no-store | Structural reject: 0 DB. Guard reject: 1 abuse transaction. Allowed: 1 abuse transaction + 1 availability transaction/function and ≤96 result rows; each abuse call upserts 1 bucket and opportunistically deletes ≤8 expired buckets; 0 Auth/provider calls                                         |
| `POST /api/bookings`               | Public; canonical supplied Origin must match; no query; canonical lowercase UUID idempotency header ≤128 B; JSON media/framing headers before the guard; raw fatal-UTF-8 strict body ≤8 KiB | Booking code, resource UUID, replay flag or fixed guard error; contact PII is never echoed                | Durable network+account buckets; Preview requires the Turnstile bundle  | Pre-guard reject: 0 DB. Valid request: 2 abuse transactions + 1 booking transaction; each abuse call upserts 1 bucket and deletes ≤8 expired buckets. Booking success remains 6 rows/7 mutations; replay 0; handled failure 1 row/2 mutations; 0 sends                                               |
| `GET /api/cron/outbox`             | Exact path/query plus canonical 32-byte bearer; Local/Test or exact registered-TEST Vercel Preview only; every other remote target fails closed                                             | Batch summary and request UUID; no message body, recipient, or provider identifier                        | One manual invocation; no-store and noindex                             | Config/auth reject: 0 DB/provider calls. Safe TEST success: ≤5 outbox claims, ≤25 eligible stored-webhook recovery attempts with persisted backoff/terminal disposition, ≤25 dead-letter alert events with one durable ack, and ≤1,000 expired abuse buckets purged; fake email/alert receivers only |
| `POST /api/newsletter/subscribe`   | Exact same-origin request, unique idempotency key, JSON framing/body ≤4 KiB, canonical email, consent, and abuse guard                                                                      | Fixed non-enumerating `202 REQUEST_ACCEPTED`; email is never echoed                                       | Durable network+account buckets; Preview requires the Turnstile bundle  | Valid request: 2 abuse transactions + 1 subscriber transaction/function; each abuse call upserts 1 bucket and deletes ≤8 expired buckets; SQL ≤2 command rows accepted exactly 1. Account-scope rejection stays fixed 202 and skips the command; 0 synchronous provider sends                        |
| `POST /api/newsletter/confirm`     | Exact same-origin request, double-submit CSRF, unique idempotency key, body ≤4 KiB, and signed expiring action token                                                                        | Fixed non-enumerating `202 REQUEST_ACCEPTED`; subscriber/token data is never echoed                       | Durable network+token buckets; Preview requires the Turnstile bundle    | Invalid token: 1 network-abuse transaction, ≤1 bucket upsert, ≤8 expiry deletes, and 0 subscriber commands. Valid token: 2 such abuse transactions + 1 action transaction/function; token-scope rejection stays fixed 202; 0 synchronous provider sends                                              |
...
two bounded bucket transactions. Every bucket transaction upserts one current
bucket and opportunistically deletes at most eight expired rows through the
expiry index with locked rows skipped; verified challenges bypass only the
configured challenge threshold, never the hard limit. The guard receives cloned
headers, not request bodies. Preview is enabled only when the reviewed Turnstile
adapter and its exact protected configuration bundle are both present;
misconfigured Preview, Production, operator, invalid, thrown, and malformed
paths return fixed `503 SERVICE_UNAVAILABLE` before public business data work.
...
All eleven routes require exact Origin/Host, double-submit CSRF, a canonical
...
| `POST /api/admin/vacations/update`        | `vacationId`, `expectedVersion`, replacement start/end spanning ≤366 dates, optional reason   | `VACATION_UPDATED` + same ID            | Locks the old/new date union (≤732); one vacation update + domain change + command insert/complete: **735 rows/736 mutations**                                         |
...
## Owner subscriber command

`POST /api/admin/subscribers/unsubscribe` accepts exactly `subscriberId` and
`expectedVersion` 1..2,147,483,647 behind the shared fresh-owner,
HMAC-binding, Origin/CSRF, query-free, idempotency, and cutover-write gates. It
performs a version-checked soft unsubscribe of one `legacy_unverified`,
`pending`, `active`, or `bounced` row. Already-unsubscribed and complained rows
return a conflict; no subscriber is deleted and no email is sent.

After the read-only cutover gate, the command uses one owner transaction with
one cutover-authorization query and one private command query capped at two
returned rows. A fresh success affects three rows with four mutations: command
insert, subscriber status/version update, PII-free domain-change insert, and
command completion. A handled not-found/version/state conflict affects one
command row twice. An exact replay mutates no rows. Physical reads remain
measurement-dependent.

| Route                                     | Strict body                                        | Response                                  | Maximum command effect                                                                                                                   |
| ----------------------------------------- | -------------------------------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /api/admin/subscribers/unsubscribe` | `subscriberId`, `expectedVersion` 1..2,147,483,647 | Unsubscribe code + subscriber ID/replayed | 1 owner transaction/2 function calls/≤3 returned rows; success **3 rows/4 mutations**; conflict **1 row/2 mutations**; replay 0; 0 sends |
