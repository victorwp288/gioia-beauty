...
create function gioia_private.owner_reschedule_block(
...
begin
  ...
  begin
    ...
    if v_entry.status <> 'active' then
      raise sqlstate 'PT409' using message = 'BLOCK_NOT_RESCHEDULABLE';
    end if;
    if v_entry.local_date = p_local_date
      and v_entry.start_minutes = p_start_minutes
      and v_entry.service_duration_minutes = p_duration_minutes
      and v_entry.buffer_minutes = p_buffer_minutes then
    ...
    set local_date = p_local_date,
        ...
        service_duration_minutes = p_duration_minutes,
    ...
    perform gioia_private.record_domain_change(
      ...
      array[
        'local_date', 'start_minutes',
        'service_duration_minutes', 'buffer_minutes'
      ]::text[]
    );
...
revoke all on function gioia_private.owner_reschedule_block(
  uuid, text, bytea, uuid, integer, date, smallint, smallint, smallint
) from public, anon, authenticated, service_role, gioia_migrator;
grant execute on function gioia_private.owner_reschedule_block(
  uuid, text, bytea, uuid, integer, date, smallint, smallint, smallint
) to app_runtime;
