...
describe("greenfield TEST Supabase CLI", () => {
  ...
  it("passes all 31 reviewed remote pgTAP files explicitly", async () => {
    const { run } = fakeRunner();
    ...
    expect(result).toMatchObject({ assertions: 475, files: suite.files });
    ...
    expect(testCalls).toHaveLength(33);
    expect(
      testCalls.every(
