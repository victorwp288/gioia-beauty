...
- The reviewed hosted pgTAP transport keeps each file in one rollback-only transaction but sends the re-subscription scenarios in `080_subscriber_commands.test.sql` and `100_verified_webhook_commands.test.sql` as two explicit query messages at their committed markers, with a 5 ms pause beyond the domain's canonical millisecond precision. This gives each compressed re-subscription a fresh PostgreSQL `statement_timestamp()` like separate user requests and the local CLI runner; every split is filename-pinned, digest-pinned, and rejected if missing, duplicated, or added to another file.
